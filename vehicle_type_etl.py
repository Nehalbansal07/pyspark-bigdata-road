...
print("🚗 Starting Vehicle Type Analysis ETL Pipeline...")
...
try:
    ...
    print("✅ Road info data loaded successfully")
    print(f"Road info records: {road_info.count()}")
    road_info.show(5, truncate=False)

except Exception as e:
    print(f"❌ Error loading road_info.csv: {e}")
    spark.stop()
    exit()
...
print("🔗 Joining vehicle data with road information...")
...
print("📊 Calculating vehicle statistics...")
joined = joined \
...
try:
    ...
    print("✅ Aggregated data saved to Parquet")
except Exception as e:
...
try:
    ...
    print("✅ Pattern analysis saved to Parquet")
except Exception as e:
...
print("\n" + "="*60)
print("📊 VEHICLE TYPE ANALYSIS SUMMARY")
print("="*60)
...
print(f"🚛 Average Truck Percentage: {avg_truck_percent:.1f}%")
print(f"🏍  Average Bike Percentage: {avg_bike_percent:.1f}%")
print(f"🚗 Average Car Percentage: {avg_car_percent:.1f}%")
print(f"📈 Maximum Vehicle Density: {max_density:.0f} vehicles/lane/day")
...
print("📊 Creating Vehicle Type Analysis Visualizations...")
...
try:
    ...
    print("❌ Data files not found. Please run ps-3.py first")
    exit()
...
print("\n🎨 Creating Visualization 1: Vehicle Type Distribution Overview...")
...
fig1.update_layout(
    title_text="🚗 VEHICLE TYPE ANALYSIS OVERVIEW",
    height=700,
    showlegend=True,
    barmode='stack'
)
...
print("🎨 Creating Visualization 2: High Truck Percentage Analysis...")
...
if not high_truck.empty:
    ...
    fig2.update_layout(
        title_text="🚛 HIGH TRUCK PERCENTAGE ANALYSIS",
        height=500,
        showlegend=False
    )
    ...
    print("⚠ No roads with high truck percentage found")

# =============================================================================
# VISUALIZATION 3: VEHICLE DENSITY HEATMAP
# =============================================================================

print("🎨 Creating Visualization 3: Vehicle Density Heatmap...")
...
fig3.update_layout(
    title="🔥 VEHICLE DENSITY HEATMAP BY CITY AND ROAD",
    xaxis_title="Road ID",
    yaxis_title="City",
    height=500
)
...
print("🎨 Creating Visualization 4: Comprehensive Road Analysis...")
...
fig4.update_layout(
    title='📊 DETAILED VEHICLE DISTRIBUTION BY ROAD (Top 15)',
    xaxis_title='Road ID',
...
print("🎨 Creating Visualization 5: Road Pattern Radar Chart...")
...
if len(road_types) > 0:
    ...
    fig5.update_layout(
        ...
        title="🔄 ROAD TYPE CHARACTERISTICS (Radar Chart)",
        height=500
    )
...
print(f"🚛 High Truck Roads (>30%): {len(df[df['avg_trucks_percent'] > 30])}")
print(f"🏍  High Bike Roads (>40%): {len(df[df['avg_bikes_percent'] > 40])}")
print(f"🚗 High Car Roads (>60%): {len(df[df['avg_cars_percent'] > 60])}")
print(f"📊 Overloaded Roads (density > 1000): {len(df[df['vehicle_density'] > 1000])}")

print(f"\n🏆 Top 5 Roads by Total Vehicles:")
top_5_total = df.nlargest(5, 'sum_total_vehicles')[['road_id', 'city', 'sum_total_vehicles']]
for _, row in top_5_total.iterrows():
    print(f"   • {row['road_id']} ({row['city']}): {row['sum_total_vehicles']:,.0f} vehicles")

print(f"\n📈 Top 5 Roads by Vehicle Density:")
top_5_density = df.nlargest(5, 'vehicle_density')[['road_id', 'city', 'vehicle_density']]
for _, row in top_5_density.iterrows():
    print(f"   • {row['road_id']} ({row['city']}): {row['vehicle_density']:.0f} vehicles/lane")
